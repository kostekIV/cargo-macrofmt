...
#[tracing::instrument(level = "debug", target = LOG_TARGET, name = "handle-event", skip_all)]
async fn handle_event(&mut self) -> Result<()> {
    Ok(())
}

#[custom::testing::test_macro(param1 = "value1", param2 = "value2", param3 = "value3")]
fn custom_test() {
    assert!(true);
}

#[some::deeply::nested::path::instrument(level = "info", name = "nested", skip_all)]
